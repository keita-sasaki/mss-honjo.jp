...
function seo_title( $title, $sep ) {
  ...
  $options = get_design_plus_option();
	...
	} elseif ( is_home() && get_post_meta( get_option( 'page_for_posts' ), 'tcd-w_meta_title', true ) ) {
  	$title = get_post_meta( get_option( 'page_for_posts' ), 'tcd-w_meta_title', true ) .  ' | ' . get_bloginfo( 'name' );
 	//} elseif ( is_home() ) {
  	//$title = esc_html(get_the_title( get_option( 'page_for_posts' ))) .  ' | ' . get_bloginfo( 'name' );
 	} elseif ( is_category() ) {
...
function get_seo_description() {
	...
	// カスタムフィールドが存在し、ブログ一覧ページの場合
	} elseif ( is_home() && get_post_meta( get_option( 'page_for_posts' ), 'tcd-w_meta_description', true ) ) {
	$trim_content = get_post_meta( get_option( 'page_for_posts' ), 'tcd-w_meta_description', true );
	$trim_content = str_replace( array( "\r\n", "\r", "\n" ), '', $trim_content );
  	$trim_content = htmlspecialchars( $trim_content );
	return esc_html( $trim_content );
    ...
    return sprintf( __( 'Post list for %s', 'tcd-w' ), get_search_query() );
  	...
  	$cat_data = get_option( "cat_$cat_id" );
		...
		if ( tag_description() ) {
			$tag_desc = strip_tags( tag_description() );
    		$tag_desc = str_replace( array( "\r\n", "\r", "\n" ), '', $tag_desc );
			return esc_html( $tag_desc );
		} else {
    		return esc_html( get_bloginfo( 'description' ) );
		}		
 	} else {
